                    ...
                    <div class="section-card border-2 border-red-200 rounded-lg p-6 bg-white transition-all duration-300 hover:shadow-lg animate-slide-up">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                            ...
                            <div class="form-group animate-fade-in-up">
                                ...
                                <div class="input-wrapper">
                                    <select name="chosen_installments" id="chosen_installments" class="form-select" required>
                                        @php
                                            $allPaymentOptions = [1, 2, 3, 4, 6, 10, 12];
                                        @endphp
                                        @foreach($allPaymentOptions as $option)
                                            <option value="{{ $option }}" {{ ($option == old('chosen_installments', $inscription->chosen_installments)) ? 'selected' : '' }}>
                                                @if($option == 1)
                            ...
                            <div class="form-group animate-fade-in-up">
                                ...
                                <div class="input-wrapper">
                                    <select name="inscrit_par" id="inscrit_par" class="form-select">
                                        <option value="">Sélectionner</option>
                                        @php
                                            $agents = ['Sara BELKASSEH', 'Ghizlane LAFKIR', 'Lamiae HAJIB', 'Abdellatif LEZHARI', 'Khalid Katkout'];
                                        @endphp
                                        @foreach($agents as $agent)
                                            <option value="{{ $agent }}" {{ old('inscrit_par', $inscription->inscrit_par) == $agent ? 'selected' : '' }}>
                                                {{ $agent }}
                                            </option>
                                        @endforeach
                                    </select>
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                // Event listener for changes on the formation dropdown
                formationSelect.addEventListener('change', function () {
                    // Update the total amount when the formation changes
                    const selectedOption = this.options[this.selectedIndex];
                    const newPrice = selectedOption.dataset.price;
                    totalAmountInput.value = newPrice;

                    // Re-populate the payment installments options based on the new formation
                    updateChosenInstallmentsOptions();
                });

                // Event listener for changes on the chosen installments dropdown
                chosenInstallmentsSelect.addEventListener('change', updatePaymentDetails);
                totalAmountInput.addEventListener('input', updatePaymentDetails);
                ...
                function updateChosenInstallmentsOptions() {
                    ...
                    try {
                        ...
                        availableOptions = [1]; // Fallback to 1 installment
                    }

                    // Get the old value to re-select it if available
                    const oldChosen = "{{ old('chosen_installments', $inscription->chosen_installments) }}";
                    let selectedValue = oldChosen;

                    // If the old value isn't in the new list, default to the first available option
                    if (!availableOptions.includes(parseInt(oldChosen))) {
                        selectedValue = availableOptions.length > 0 ? availableOptions[0] : 1;
                    }
                    ...
                    // Trigger the payment details update after changing options
                    updatePaymentDetails();
                }

                function updatePaymentDetails() {
                    const totalAmount = parseFloat(totalAmountInput.value) || 0;
                    const installments = parseInt(chosenInstallmentsSelect.value) || 1;
                    const summaryBox = document.getElementById('payment-details-summary');
                    ...
                    if (totalAmount > 0 && installments > 1) {
                        const firstInstallmentAmount = totalAmount / installments;
                        const remainingInstallmentsAmount = (totalAmount - firstInstallmentAmount);

                        firstPaymentDetails.textContent = `Montant de la première échéance : ${firstInstallmentAmount.toFixed(2)} DH`;
                        remainingPaymentsDetails.textContent = `Le solde restant de ${remainingInstallmentsAmount.toFixed(2)} DH sera réparti sur ${installments - 1} acomptes.`;
                        summaryBox.style.display = 'block';
                    } else if (totalAmount > 0 && installments === 1) {
                        firstPaymentDetails.textContent = `Paiement complet : ${totalAmount.toFixed(2)} DH`;
