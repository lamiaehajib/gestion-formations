...
<head>
    ...
    <title>Nouveau paiement soumis</title>
    <style>
        ...
        <tr>
            <td>
                ...
                <table class="content" role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%">
                    <tr>
                        <td>
                            <h1 class="heading">Nouveau paiement soumis</h1>
                            <p>
                                Un nouveau paiement a été soumis par l'étudiant <strong>{{ $payment->inscription->user->name ?? 'N/A' }}</strong>.
                            </p>
                            
                            <div class="details-box">
                                <p><strong>Formation :</strong> {{ $payment->inscription->formation->title ?? 'N/A' }}</p>
                                <p><strong>Montant payé :</strong> <span style="color: #C2185B;">{{ number_format($payment->amount, 2) }} DH</span></p>
                                <p><strong>Méthode de paiement :</strong> {{ $payment->payment_method }}</p>
                                <p><strong>Statut :</strong> <span style="color: #ef4444;">{{ ucfirst($payment->status) }}</span></p>
                                @if($payment->receipt_path)
                                    <p style="margin-top: 15px;"><a href="{{ asset('storage/' . $payment->receipt_path) }}" class="btn">Voir la preuve de paiement</a></p>
                                @endif
                            </div>

                            <p style="margin-top: 30px;">
                                Veuillez vous connecter à votre panneau d'administration pour vérifier ce paiement.
                            </p>
