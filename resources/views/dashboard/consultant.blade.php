...
@section('content')
    <div class="dashboard-body">
        <div class="dashboard-container">
            ...
            <div class="stats-grid">
                ...
                <div class="stat-card">
                    <div class="stat-content">
                        <div class="stat-icon" style="background: linear-gradient(45deg, var(--accent-purple), #9B59B6);"><i class="fas fa-tasks"></i></div>
                        <div class="stat-info">
                            <div class="stat-number" style="background: linear-gradient(45deg, var(--accent-purple), #9B59B6); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">{{ $totalModules }}</div>
                            <div class="stat-label">Total Modules Assignés</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="stat-card">
                    <div class="stat-content">
                        <div class="stat-icon" style="background: linear-gradient(45deg, #28B463, #2ECC71);"><i class="fas fa-chart-bar"></i></div>
                        <div class="stat-info">
                            <div class="stat-number" style="background: linear-gradient(45deg, #28B463, #2ECC71); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">{{ $overallAverageProgress }}%</div>
                            <div class="stat-label">Progression Moyenne des Modules</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            {{-- Global Module Progress Chart and Module Progress Statistics --}}
                            ...
                            <div class="table-responsive">
                                <table class="table table-subtle-depth">
                                    <thead>
                                        <tr>
                                            <th>Cours</th>
                                            <th>Module</th> {{-- Correction: Module au lieu de Formation --}}
                                            <th>Heure</th>
                                            <th>Lien Zoom</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach($coursesToday as $course)
                                            <tr>
                                                ...
                                                <td>
                                                    {{-- Utiliser la relation 'module' chargée dans le contrôleur --}}
                                                    @if($course->module)
                        ...
                        @else
                            <div class="table-responsive">
                                <table class="table table-subtle-depth">
                                    <thead>
                                        <tr>
                                            <th>Cours</th> {{-- Correction: Cours au lieu de Formation --}}
                                            <th>Module</th> {{-- Correction: Module au lieu de Formation --}}
                                            <th>Date</th>
                                            <th>Heure</th>
                                            <th>Actions</th>
                                    ...
                                    <tbody>
                                        @foreach($upcomingCourses as $course)
                                            <tr>
                                                <td><strong>{{ $course->title }}</strong></td> {{-- Afficher le titre du cours --}}
                                                <td>
                                                    {{-- Utiliser la relation 'module' chargée dans le contrôleur --}}
                                                    @if($course->module)
                                                        <span class="badge bg-info">{{ $course->module->title }}</span>
            ...
            <div class="row">
                            ...
                            <div class="table-responsive">
                                <table class="table table-subtle-depth">
                                    <thead>
                                        <tr>
                                            <th>Cours</th>
                                            <th>Module</th> {{-- Correction: Module au lieu de Cours --}}
                                            <th>Ancienne Date</th>
                                            <th>Nouvelle Date</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach($recentReschedules as $reschedule)
                                            <tr>
                                                ...
                                                <td>
                                                    {{-- Utiliser la relation 'course.module' chargée dans le contrôleur --}}
                                                    @if(optional($reschedule->course)->module)
                ...
                {{-- Espace vide pour équilibrage ou pour ajouter un autre composant --}}
                <div class="col-lg-6 mb-4">
                    {{-- Ici, vous pouvez insérer un autre composant si nécessaire, sinon il restera vide pour le design --}}
                </div>
            </div>
...
@push('scripts')
    {{-- Assurez-vous d'avoir Chart.js chargé avant ce script --}}
    <script src="https://cdn.jsdelivr.net/npm/chart.js@4.4.2/dist/chart.umd.min.js"></script>
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            
            // --- Global Modules Progress Chart (Bar Chart) ---
            const globalModulesProgressCtx = document.getElementById('globalModulesProgressChart');
            if (globalModulesProgressCtx && {{ $totalModules > 0 ? 'true' : 'false' }}) {
                new Chart(globalModulesProgressCtx, {
                    type: 'bar',
                    data: {
                        ...
                        datasets: [{
                            ...
                            borderColor: @json($globalModulesChart['backgroundColor']),
                            borderWidth: 1
                        }]
                    },
                    options: {
            ...
            // --- Module Progress Stats Chart (Doughnut Chart) ---
            const moduleStatsCtx = document.getElementById('moduleStatsChart');
            if (moduleStatsCtx && {{ $totalModules > 0 ? 'true' : 'false' }}) {
                new Chart(moduleStatsCtx, {
                    ...
                    data: {
                        ...
                        datasets: [{
                            data: [
                                {{ $moduleProgressStats['completed']['count'] }},
                                {{ $moduleProgressStats['in_progress']['count'] }},
                                {{ $moduleProgressStats['not_started']['count'] }}
                            ],
            ...
            // --- Courses Trend Chart (Line Chart) ---
            const coursesTrendCtx = document.getElementById('coursesTrendChart');
            if (coursesTrendCtx && @json($coursesTrend->isNotEmpty())) {
                const trendData = @json($coursesTrend);
                const labels = trendData.map(item => {
                    const [year, month] = item.month.split('-');
                    // Conversion du numéro de mois en nom pour un meilleur affichage
                    const date = new Date(year, month - 1);
                    return date.toLocaleString('fr-FR', { month: 'long', year: 'numeric' });
                });
                const data = trendData.map(item => item.total_courses);

                new Chart(coursesTrendCtx, {
                    type: 'line',
                    data: {
                        labels: labels,
                        datasets: [{
                            label: 'Total Cours Créés',
                            data: data,
                            backgroundColor: 'rgba(52, 152, 219, 0.2)', // Primary Blue transparent
                            borderColor: '#3498DB', // Primary Blue
                            borderWidth: 2,
                            tension: 0.3,
                            fill: true,
                            pointRadius: 5
