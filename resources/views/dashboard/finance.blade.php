...
@push('styles')
{{-- Include the same CSS as the Admin Dashboard for a consistent look --}}
...
<div class="dashboard-body">
    <div class="dashboard-container">
        ...
        <div class="card-base header-card">
            <h1><i class="fa-solid fa-file-invoice-dollar"></i> Tableau de Bord Financier</h1>
            <p class="header-subtitle">Aperçu complet des revenus et des paiements</p>
        ...
        <div class="stats-grid">
            <div class="stat-card">
                ...
                <div class="stat-detail positive">
                    <i class="fa-solid fa-arrow-up"></i> +{{ number_format($paidToday, 2) }} DH payés aujourd'hui
                </div>
            </div>

            <div class="stat-card">
                <div class="stat-icon icon-pending"><i class="fa-solid fa-clock-rotate-left"></i></div>
                <div class="stat-number">{{ number_format($pendingPayments, 2) }} DH</div>
                <div class="stat-label">Paiements en Attente</div>
                <div class="stat-detail info">
                    <i class="fa-solid fa-hand-holding-dollar"></i> Montant total en attente
                </div>
            </div>

            <div class="stat-card">
                <div class="stat-icon icon-overdue"><i class="fa-solid fa-triangle-exclamation"></i></div>
                <div class="stat-number">{{ number_format($overduePayments, 2) }} DH</div>
                <div class="stat-label">Paiements en Retard</div>
                <div class="stat-detail danger">
                    <i class="fa-solid fa-user-xmark"></i> {{ number_format($overdueInscriptions->count()) }} étudiants en retard
                </div>
            </div>

            <div class="stat-card">
                <div class="stat-icon icon-avg-payment"><i class="fa-solid fa-wallet"></i></div>
                <div class="stat-number">{{ number_format($averagePaymentAmount, 2) }} DH</div>
                <div class="stat-label">Paiement Moyen</div>
                <div class="stat-detail info">
                    <i class="fa-solid fa-chart-line"></i> Montant moyen par transaction
                </div>
            </div>
        </div>
        ...
        <div class="quick-actions-container">
            <div class="quick-actions-wrapper">
                ...
                <a href="{{ route('inscriptions.index') }}" class="quick-action-btn"><i class="fa-solid fa-user-plus"></i> Gérer les Inscriptions</a>
                <a href="{{ route('payments.index') }}" class="quick-action-btn"><i class="fa-solid fa-money-bill-transfer"></i> Voir l'historique des Paiements</a>
            </div>
        </div>
        ...
        <div class="charts-grid">
            ...
            <div class="card-base chart-card">
                <h3 class="chart-title"><i class="fa-solid fa-chart-bar"></i> Paiements par Méthode</h3>
                <canvas id="paymentsByMethodChart"></canvas>
            </div>
        </div>

        {{-- Lists for overdue and upcoming payments --}}
        <div class="lists-grid">
            <div class="card-base list-card">
                <h3 class="list-title"><i class="fa-solid fa-hourglass-end"></i> Étudiants en Retard de Paiement</h3>
                @if($overdueInscriptions->count() > 0)
                    @foreach($overdueInscriptions as $inscription)
                    <div class="list-item">
                        <div>
                            <strong>{{ $inscription->user->name ?? 'N/A' }}</strong>
                            <div>{{ $inscription->formation->title ?? 'N/A' }}</div>
                        </div>
                        <div style="text-align: right;">
                            <div style="font-weight: 700; color: #dc2626; font-size: 16px;">
                                {{ number_format($inscription->next_installment_amount, 2) }} DH
                            </div>
                            <div style="font-size: 12px; color: #dc2626;">Dû le : {{ $inscription->next_installment_due_date ? Carbon\Carbon::parse($inscription->next_installment_due_date)->format('d M, Y') : 'N/A' }}</div>
                        </div>
                    </div>
                    @endforeach
                @else
            ...
            <div class="card-base list-card">
                <h3 class="list-title"><i class="fa-solid fa-calendar-check"></i> Paiements à Venir (7 jours)</h3>
                @if($upcomingPaymentsDue->count() > 0)
                    @foreach($upcomingPaymentsDue as $inscription)
                    <div class="list-item">
                        <div>
                            <strong>{{ $inscription->user->name ?? 'N/A' }}</strong>
                            <div>{{ $inscription->formation->title ?? 'N/A' }}</div>
                        ...
                        <div style="text-align: right;">
                            <div style="font-weight: 700; color: #f59e0b; font-size: 16px;">
                                {{ number_format($inscription->amount_per_installment, 2) }} DH
                            </div>
                            <div style="font-size: 12px; color: #f59e0b;">Dû le : {{ $inscription->next_installment_due_date ? Carbon\Carbon::parse($inscription->next_installment_due_date)->format('d M, Y') : 'N/A' }}</div>
                        </div>
                    </div>
                    @endforeach
                @else
                    <div class="empty-state">
                        ...
                        <p>Aucun paiement prévu dans les 7 prochains jours.</p>
                    </div>
                @endif
            </div>

            <div class="card-base list-card">
                <h3 class="list-title"><i class="fa-solid fa-money-check-dollar"></i> Derniers Paiements Enregistrés</h3>
                @if($recentPayments->count() > 0)
                    @foreach($recentPayments as $payment)
                    <div class="list-item">
                        <div>
                            <strong>{{ $payment->inscription->user->name ?? 'N/A' }}</strong>
                            <div>{{ $payment->inscription->formation->title ?? 'N/A' }}</div>
                            <div style="font-size: 12px; color: #6b7280;">
                                Payé le : {{ $payment->created_at->format('d M, Y H:i') }}
                            </div>
                        </div>
    ...
    new Chart(monthlyRevenueCtx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                borderColor: 'var(--content-color-start)',
                backgroundColor: (context) => {
                    ...
                    return createGradient(ctx, chartArea, [
                        { offset: 0, color: 'rgba(211, 47, 47, 0.6)' },
                ...
                tension: 0.4,
                borderWidth: 4,
                pointBackgroundColor: 'var(--content-color-end)',
    ...
    new Chart(paymentsByMethodCtx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: [
                    '#D32F2F', '#C2185B', '#FF6F6F', '#E57373', '#FFCDD2'
                ],
                borderRadius: 10,
                borderSkipped: false,
